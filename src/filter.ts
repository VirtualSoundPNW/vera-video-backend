...
/** Decisive: the venue's actual name, however it is punctuated or hashtagged. */
const STRONG_POSITIVE = [/\bvera project\b/, /\bveraproject\b/];
const STRONG_POSITIVE_SCORE = 5;

/**
 * Supporting context. Only counted when "vera" appears at all, so a generic
 * Seattle show does not drift into the catalog. Capped so that stacking
 * weak signals cannot outweigh a decisive negative.
 */
const CONTEXT_TERMS = [
  /\ball ages\b/,
  /\ballages\b/,
 ...
 * essentially never co-occur with the venue.
 */
...
/** Nothing about Vera at all — most likely an unrelated upload from an allowed-adjacent channel. */
...
export function scoreCandidate(candidate: Candidate): { score: number; reason: string } {
  ...
  if (!VERA.test(text) && !/\bveraproject\b/.test(text)) {
    return { score: NO_MENTION_SCORE, reason: "no mention of vera" };
  }
